...
Сценарий: 703_Проверка_документа_Расход_товара
...
* Проверка введенных значений
	...
	Тогда таблица "Товары" стала равной:
		...
		| '1' | 'Молоко'  | '40,00'  | '2,00'       | '80,00' |
		| '2' | 'Хлеб'    | '20,00'  | '3,00'       | '60,00' |
...
Сценарий: проверка движений документа Расход товара по регистру взаиморасчетов с контрагентами
	...
	Тогда таблица "Список" стала равной:
		...
		| '$$ДатаДокумента$$' | '$$РегистраторДокумента$$' | '1'            | 'Магазин "Продукты"' | '140,00' | ''       |
	И я закрываю все окна клиентского приложения
...
Сценарий: проверка движений документа Расход товара по регистру продаж
	...
	Тогда таблица "Список" стала равной:
		...
		| '$$ДатаДокумента$$' | '$$РегистраторДокумента$$' | '1'            | 'Магазин "Продукты"' | 'Молоко' | '2,00'       | '80,00' |
		| '$$ДатаДокумента$$' | '$$РегистраторДокумента$$' | '2'            | 'Магазин "Продукты"' | 'Хлеб'   | '3,00'       | '60,00' |
	И я закрываю все окна клиентского приложения
...
Сценарий: проверка движений документа Расход товара по регистру товарных запасов
	...
	Тогда таблица "Список" стала равной:
		...
		| '$$ДатаДокумента$$' | 'Большой' | '$$РегистраторДокумента$$' | '1'            | 'Молоко' | '2,00'       |
		| '$$ДатаДокумента$$' | 'Большой' | '$$РегистраторДокумента$$' | '2'            | 'Хлеб'   | '3,00'       |
	И я закрываю все окна клиентского приложения
...
* Проверка печатной формы
	Тогда табличный документ "SpreadsheetDocument" содержит строки:
		...
		| 'Склад'         | 'Большой'                       | ''           | ''      |
		| 'Сумма'         | '140 рублей (Сто сорок рублей)' | ''           | ''      |
		| 'Вес'           | ''                              | ''           | ''      |
		| ''              | ''                              | ''           | ''      |
